#include <iostream>
using namespace std;
//creating a structure for the player's information.
struct playerInfo
{
	char playerName[81];
	char playerID;
...
int main()
{
	...
	win = 0;
	again = 0;
    //display board on each turn of the user after updation.
	DisplayBoard( board );
	do
	{
...
//getting input form the players.
int PlayerDrop( char board[][10], playerInfo activePlayer )
{
	int dropChoice;
	do
...
//check weather the downward space if the number inputted is correct or not.
void CheckBellow ( char board[][10], playerInfo activePlayer, int dropChoice )
{
	int length, turn;
	length = 6;
...
//checking the conditions for winner.
int CheckFour ( char board[][10], playerInfo activePlayer )
{
	char XO;
	int win;
...
//condition if the board is full then other moves can't be taken and game is finished.
int FullBoard( char board[][10] )
{
	int full;
	full = 0;
...
// if the game ends and after complition if the user wants to restart the game, this will not take the names of the persons when you restart the game, you can customise it if you want.
int restart ( char board[][10] )
{
	int restart;
